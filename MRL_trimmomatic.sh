...
THREADS=$2
fastq_skeleton="/local/projects-t2/HRBV/MRL/MRL_"$sample'/'$sample
...
/home/ksieber/bin/java -jar ~/bin/Trimmomatic-0.33/trimmomatic-0.33.jar PE -threads $THREADS -phred33 -trimlog $fastq_skeleton"_trimmomatic.log" $fastq_skeleton"_R1.fastq.gz" $fastq_skeleton"_R2.fastq.gz" $fastq_skeleton"_R1_trimmed_paired.fastq" $fastq_skeleton"_R1_trimmed_unpaired.fastq" $fastq_skeleton"_R2_trimmed_paired.fastq" $fastq_skeleton"_R2_trimmed_unpaired.fastq" ILLUMINACLIP:$adaptor_dir:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
